    ...
    for term in data:
        ...
        for video in video_list:
            ...
            if video_id not in missing_videos:
                ...
                vidcap = cv2.VideoCapture(f"../wlasl_dataset/videos/{video_id}.mp4")
                ...
                while success:
                    if start_frame <= frameIndex <= end_frame:
                        ...
                        cv2.imwrite(
                            f"{PROCESSED_FRAMES_PATH}{word}/{video_id}/img_{index:03}.jpg",
                            image,
                        )
                        index += 1
                    success, image = vidcap.read()
                ...
                annotations_file.write(f"{word}/{video_id} 1 {index - 1} {label_id}\n")
        label_id += 1
